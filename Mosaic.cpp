...
bool Mosaic::RowFull() {
  ...
  for (size_t i = 0; i < MOSAIC_DIM && !any_row_full; i++) {
    bool row_full = true;
    for (size_t j = 0; j < MOSAIC_DIM; j++) {
      if (this->wall.at(i).at(j) == nullptr) row_full = false;
    }
    if (row_full) any_row_full = true;
  }
  return any_row_full;
}
...
int Mosaic::EndGameWallScore() {
  ...
  for (size_t i = 0; i < MOSAIC_DIM; i++) {
    ...
    for (size_t j = 0; j < MOSAIC_DIM; j++) {
      if (this->wall.at(i).at(j) == nullptr) row_full = false;
      if (this->wall.at(j).at(i) == nullptr) col_full = false;
      int colour = (int)this->wall.at(i).at(j)->GetColour();
      colour_tally.at(colour)++;
      if (colour_tally.at(colour) == MOSAIC_DIM) score += 10;
    }
    if (row_full) score += ROW_FULL_BONUS;
    if (col_full) score += COL_FULL_BONUS;
  }
  return score;
}
...
std::string Mosaic::FloorLineToString() {
  ...
  while (fl.size() < FLOOR_SZ) fl.append(".");
  return fl + "\n";
}
...
std::string Mosaic::ToString() {
  ...
  // floor line (broken)
  ss << "broken: " << this->floor_line->ToString();
  return ss.str();
}